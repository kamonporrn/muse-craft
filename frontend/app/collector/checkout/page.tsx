...
function PaymentQRPageContent() {
  ...
  // Demo params via query: ?title=Ocean%27s%20Whisper&amount=1000&mins=15
  const title = sp.get("title") || "Ocean’s Whisper";
  const amount = Number(sp.get("amount") || 1000); // THB
  const minutes = Number(sp.get("mins") || 15);    // time limit (minutes)
  ...
  const markPaid = () => {
    if (expired) {
      alert("This QR has expired. Please create a new order.");
      return;
    }
    // Demo behavior
    alert("Payment received (demo). Redirecting to your order…");
        ...
        <section className="mt-6 rounded-2xl bg-white shadow-xl p-6 md:p-8">
          ...
          <div className="mt-8 rounded-xl bg-purple-50 p-4 text-sm text-purple-900">
            <div className="mb-2 inline-flex items-center gap-2 font-semibold">
              <Info className="w-4 h-4" /> How to pay with PromptPay
            </div>
            <ol className="list-decimal pl-5 space-y-1 text-gray-700">
              <li>Open your mobile banking app.</li>
              <li>Select “Scan” and scan the QR above.</li>
              <li>Confirm the amount exactly as shown and pay.</li>
              <li>Return to this page and click “I have paid”.</li>
            </ol>
            {expired && (
              <p className="mt-3 font-semibold text-red-600">
                * This QR has expired. Please place a new order.
              </p>
            )}
          </div>
        </section>
