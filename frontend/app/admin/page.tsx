...
function buildSmoothPath(values: number[], W: number, H: number, pad: number, min: number, max: number) {
...
export default function AdminDashboard() {
  ...
  /** Hover tooltip (ชี้ดูค่าของวันนั้นทุกซีรีส์) */
  ...
  return (
    <div className="min-h-screen bg-[#efe2fb]">
      ...
      <main className="mx-auto max-w-6xl px-4 py-6">
        ...
        {/* STAT CARDS + Δ สัปดาห์นี้ */}
        <section className="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {nowStats.map((s) => (
            <div
              key={s.key}
              className="rounded-xl bg-white shadow-[0_6px_0_rgba(0,0,0,0.04)] ring-1 ring-purple-100 p-4"
            >
              <div className="text-sm font-semibold text-gray-700">{s.label}</div>
              <div className="mt-1 text-3xl font-extrabold" style={{ color: s.color }}>
                {s.value}
              </div>
              <div className="mt-2 inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium"
                   style={{
                     ...
                     border: `1px solid ${s.delta >= 0 ? "#10b98133" : "#ef444433"}`
                   }}>
        ...
        {/* CHART: รายสัปดาห์ (ชี้ดูขึ้นค่าทุกซีรีส์) */}
        <section className="mt-6 rounded-xl bg-white ring-1 ring-purple-100 p-4 shadow-[0_6px_0_rgba(0,0,0,0.04)]">
          <div className="mb-3 flex items-center justify-between">
            <div className="text-sm font-semibold text-gray-800">Weekly Trend (by metric)</div>
            {/* Legend */}
            <div className="flex flex-wrap items-center gap-3 text-xs">
              {series.map(s => (
                <span key={s.name} className="inline-flex items-center gap-2">
                  <i className="inline-block h-2.5 w-2.5 rounded-full" style={{ background: s.color }} />
          ...
          <div className="relative overflow-hidden rounded-lg bg-white ring-1 ring-gray-100">
            ...
            {hoverIdx !== null && (
              <div className="absolute left-1/2 top-3 z-10 -translate-x-1/2 rounded-md border border-gray-200 bg-white px-3 py-2 text-xs shadow">
                <div className="mb-1 font-semibold text-gray-700 text-center">{DAYS[hoverIdx]}</div>
            ...
            >
              {/* grid lines */}
              {[1,2,3,4,5].map((i)=>(
                <line key={i} x1="40" x2="880" y1={40+i*40} y2={40+i*40} stroke="#eee" />
              ))}
              ...
              {hoverIdx !== null && (() => {
                ...
                return <line x1={x} x2={x} y1={28} y2={252} stroke="#e5e7eb" strokeDasharray="4 4" />;
              })()}

              {/* series lines + points */}
              {chart.map((s, idx)=>(
                <g key={idx}>
                  <path d={s.d} fill="none" stroke={s.color} strokeWidth="3" />
                  {s.points.map((p: any, i: number)=>(
                    <circle key={i} cx={p.cx} cy={p.cy} r={hoverIdx === i ? 5 : 3.5}
                            fill={s.color} opacity={hoverIdx === null || hoverIdx === i ? 1 : 0.35}/>
                  ))}
                </g>
              ))}
