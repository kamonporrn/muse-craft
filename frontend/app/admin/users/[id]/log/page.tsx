...
export default function CreatorArtworksPage({ params }: { params: { id: string } }) {
  // guard & profile
  ...
  return (
    <div className="min-h-screen bg-[#efe2fb]">
      ...
      <main className="mx-auto max-w-6xl px-4 py-6">
        <h1 className="text-2xl font-bold text-gray-900">Artworks by {user.name}</h1>

        <div className="mt-4 grid grid-cols-1 gap-5 sm:grid-cols-2 md:grid-cols-3">
          {items.map((p) => (
            <article key={p.name} className="rounded-xl bg-white p-3 ring-1 ring-purple-100 shadow-sm">
              <div className="overflow-hidden rounded-lg">
                <Image src={p.img} alt={p.name} width={800} height={600} className="h-44 w-full object-cover" />
              </div>
              <div className="mt-2 text-sm font-semibold text-gray-800">{p.name}</div>
              <div className="text-xs text-gray-500">{p.category}</div>
              <div className="mt-1 text-sm font-bold text-gray-900">
                {p.price.toLocaleString("th-TH", { style: "currency", currency: "THB", maximumFractionDigits: 0 })}
              </div>
            </article>
          ))}
