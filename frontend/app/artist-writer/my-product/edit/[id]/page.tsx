...
import { fetchProductById, updateProduct, fetchCurrentUser } from "@/lib/api";
...
export default function EditProductPage() {
    ...
    const loadProduct = async () => {
      ...
      try {
        ...
        if (product) {
          ...
          setCategory(product.category.toLowerCase().replace(/\s+/g, '-').replace(/\(e-book\)/g, 'literature'));
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      let imagePath = imagePreview; // Keep existing image if no new image uploaded
      if (image) {
        try {
      ...
      // Update product via API - set status to pending for re-approval
      ...
      if (updated) {
        // Redirect to status view page (same as create product flow)
        router.push(`/artist-writer/my-product?view=status&productId=${productId}`);
      } else {
    ...
    return (
      <div className="flex items-center justify-center">
        <div className="bg-white rounded-2xl shadow p-9 w-full max-w-4xl">
          ...
          <button
            onClick={() => router.push('/artist-writer/my-product')}
